...
namespace Mirror
{
    ...
    public class NetworkRoomManager : NetworkManager
    {
        ...
        List<string> instantiatedObjectNames = new List<string>();
        ...
        void SceneLoadedForPlayer(NetworkConnectionToClient conn, GameObject roomPlayer)
        {
            ...
            if (gamePlayer == null && booler == true)
            {
                ...
                gamePlayer = startPos != null
                    ...
                    gamePlayer.name = "MyPlayerObject2";
                    //instantiatedObjectNames.Add(gamePlayer.name);
                    
                    // Save the list of names to a text file.
                    File.WriteAllText("false.txt", gamePlayer.name);
            }
            if (gamePlayer == null && booler == false)
            {
                // get start position from base class
                Transform startPos = GetStartPosition();
                gamePlayer = startPos != null
                    ? Instantiate(playerPrefab, startPos.position, startPos.rotation)
                    : Instantiate(playerPrefab, Vector3.zero, Quaternion.identity);
                    gamePlayer.name = "MyPlayerObject1";
                    //instantiatedObjectNames.Add(gamePlayer.name);
                    
                    // Save the list of names to a text file.
                    File.WriteAllText("true.txt", gamePlayer.name);
                    booler = true;
            }
